...
public class AddStock {
	...
	public AddStock() {
		...
		JFrame f = new JFrame("Add Product");// creating instance of JFrame
		...
		b1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (pName.equals("")) {
					...
					Product p = new Product(pName, pQuantity, pRate);

					int a = 0;

					try {
						a = new ProductDao().insert(p);
					} catch (ClassNotFoundException | SQLException e1) {
						e1.printStackTrace();
					}

					if (a > 0) {
						JOptionPane.showMessageDialog(f, "Successfully! Record save");
